...
public class AllEqual2D {
   ...
   public static boolean equalValues(int[][] values) {
      ...
      // Use a nested for loop (i.e. two for loops, one inside the other) that goes
      // through
      // each integer element stored in the 2D array values[][] and compares each to
      // elem
      // As soon as one comparison is false then return false. If all comparisons are
      // true then
      // the loops finish and outside of the nested loops, return true.
      // Hint: values.length stores the number of rows in the 2D array,
      // values[i].length stores the
